...
export const comment = (() => {
    ...
    const onNullComment = () => {
        const desc = lang
            .on('vi', 'Hãy là người đầu tiên gửi lời chúc đến cô dâu chú rể nhé!')
            .on('en', 'Be the first to send your wishes to the couple!')
            .get();
    ...
    const showOrHide = (button) => {
        ...
        button.innerText = isShow ? `Xem ${ids.length} phản hồi` : 'Ẩn phản hồi';
    ...
    const showMore = (anchor, uuid) => {
        ...
        anchor.innerText = isCollapsed ? 'Thu gọn' : 'Xem thêm';
    ...
    const update = async (button) => {
        ...
        if (!gifIsOpen && form.value?.trim().length === 0) {
            util.notify('Vui lòng nhập lời chúc.').warning();
            return;
        }
    ...
    const send = async (button) => {
        ...
        if (nameValue.length === 0) {
            util.notify('Vui lòng nhập tên của bạn.').warning();
        ...
        if (!id && presence && presence.value === '0') {
            util.notify('Vui lòng chọn xác nhận tham dự.').warning();
            return;
        }
        ...
        if (gifIsOpen && !gifId) {
            util.notify('Vui lòng chọn ảnh động.').warning();
            return;
        }
    ...
    const cancel = async (button, id) => {
        ...
        if (gif.isOpen(id) && ((!gif.getResultId(id) && isChecklist === isPresent) || util.ask('Bạn chắc chắn chứ?'))) {
            await gif.remove(id);
            removeInnerForm(id);
            return;
        }
        ...
        const form = document.getElementById(`form-inner-${id}`);
        if (form.value.length === 0 || (util.base64Encode(form.value) === form.getAttribute('data-original') && isChecklist === isPresent) || util.ask('Bạn chắc chắn chứ?')) {
            removeInnerForm(id);
            return;
        }
