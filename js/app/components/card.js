...
export const card = (() => {
    ...
    /**
     * Đổi mốc thời gian sang tiếng Việt: "5 phút trước", "2 ngày trước"…
     * @param {string} moc
     * @returns {string}
     */
    const thoiGianViet = (moc) => {
        if (!moc) {
            return '';
        }

        const t = new Date(moc);
        ...
        for (const [don_vi, ten] of moc_tg) {
            if (giay >= don_vi) {
                return `${Math.floor(giay / don_vi)} ${ten} trước`;
            }
        }
    ...
    const renderAction = (c) => {
        ...
        if (session.isAdmin()) {
            action += `<button style="font-size: 0.8rem;" onclick="undangan.comment.remove(this)" data-uuid="${c.uuid}" class="btn btn-sm btn-outline-auto rounded-4 py-0 me-1 shadow-sm" data-own="${c.own}" data-offline-disabled="false">Xoá</button>`;
        } else if (owns.has(c.uuid) && config.get('can_delete') !== false) {
            action += `<button style="font-size: 0.8rem;" onclick="undangan.comment.remove(this)" data-uuid="${c.uuid}" class="btn btn-sm btn-outline-auto rounded-4 py-0 me-1 shadow-sm" data-offline-disabled="false">Xoá</button>`;
        }
    ...
    const renderReadMore = (uuid, uuids) => {
        ...
        return `<a class="text-theme-auto" style="font-size: 0.8rem;" onclick="undangan.comment.showOrHide(this)" data-uuid="${uuid}" data-uuids="${util.escapeHtml(uuids.join(','))}" data-show="${hasId ? 'true' : 'false'}" role="button" class="me-auto ms-1 py-0">${hasId ? 'Ẩn phản hồi' : `Xem ${uuids.length} phản hồi`}</a>`;
    };
    ...
    const renderBody = async (c) => {
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <small class="text-theme-auto m-0 p-0" style="font-size: 0.75rem;">${thoiGianViet(c.created_at)}</small>
        </div>
        <hr class="my-1">`;
        ...
        ${moreMaxLength ? `<p class="d-block mb-2 mt-0 mx-0 p-0"><a class="text-theme-auto" role="button" style="font-size: 0.85rem;" data-show="false" onclick="undangan.comment.showMore(this, '${c.uuid}')">Xem thêm</a></p>` : ''}`;
    };
    ...
    const renderEdit = (id, presence, is_parent, is_gif) => {
        ...
        inner.id = `inner-${id}`;
        const template = `
        <p class="my-1 mx-0 p-0" style="font-size: 0.95rem;"><i class="fa-solid fa-pen me-2"></i>Sửa lời chúc</p>
        ${!is_parent ? '' : `
        <select class="form-select shadow-sm mb-2 rounded-4" id="form-inner-presence-${id}" data-offline-disabled="false">
            <option value="1" ${presence ? 'selected' : ''}>&#9989; Có, tôi sẽ đến</option>
            <option value="2" ${presence ? '' : 'selected'}>&#10060; Rất tiếc, tôi bận</option>
        </select>`}
