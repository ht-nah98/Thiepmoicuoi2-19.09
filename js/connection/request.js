...
export const cacheWrapper = (cacheObject) => {
    ...
    const has = (input) => cacheObject.match(input).then((res) => {
        ...
        // Thiếu Cache-Control hoặc max-age thì coi như còn hạn,
        // tránh ném lỗi khi đọc thuộc tính của null.
        const maxAge = res.headers.get('Cache-Control')?.match(/max-age=(\d+)/)?.at(1);
        const ngayGui = Date.parse(res.headers.get('Date') ?? '');

        if (!maxAge || Number.isNaN(ngayGui)) {
            return res;
        }

        return Date.now() > (ngayGui + parseInt(maxAge, 10) * 1000) ? null : res;
    });
...
export const request = (method, path) => {
    ...
    const req = {
        signal: ac.signal,
        // Thiệp 2 gọi API của Thiệp 1 (khác tên miền). Backend trả CORS dạng '*',
        // nên KHÔNG được dùng credentials 'include' (trình duyệt sẽ chặn preflight).
        // App xác thực qua header x-access-key/Authorization, không dùng cookie,
        // nên 'omit' là an toàn và cho phép cross-origin hoạt động.
        credentials: 'omit',
    ...
    */
    ...
    return {
        ...
        send(transform = null) {
            ...
            // data-url có thể là đường dẫn tương đối ("/") khi API nằm cùng tên miền,
            // nên lấy địa chỉ trang hiện tại làm gốc cho URL tuyệt đối.
            const goc = new URL(
                document.body.getAttribute('data-url') ?? '/',
                window.location.origin,
            );

            return baseFetch(new URL(path.replace(/^\//, ''), goc)).then((res) => {
                if (downName && res.ok) {
                    return {
                        code: res.status,
         ...
         */
        token(token) {
            // Không có token (backend riêng không bắt buộc) thì bỏ qua
            if (!token) {
                return this;
            }
