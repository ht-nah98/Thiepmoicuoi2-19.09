export const util = (() => {
    ...
    const notify = (message) => {
        /**
         * Hiện thông báo trượt lên từ đáy màn hình, tự biến mất sau 3.2 giây.
         * Dịu hơn window.alert vốn chặn cả trang.
         * @param {string} loai
         * @returns {void}
         */
        const exec = (loai) => {
            document.querySelectorAll('.thong-bao').forEach((e) => e.remove());

            const hop = document.createElement('div');
            hop.className = `thong-bao thong-bao-${loai}`;
            hop.setAttribute('role', 'status');
            hop.textContent = String(message);
            document.body.appendChild(hop);

            requestAnimationFrame(() => hop.classList.add('hien'));
        ...
        return {
            success: () => exec('thanh-cong'),
            error: () => exec('loi'),
            warning: () => exec('canh-bao'),
            info: () => exec('tin'),
            custom: () => exec('tin'),
        };
    };
    ...
    const changeOpacity = (el, isUp, step = 0.05) => new Promise((res) => {
        ...
        // Tab ở chế độ nền thì trình duyệt tạm dừng requestAnimationFrame,
        // hiệu ứng sẽ treo mãi. Khi đó đặt thẳng giá trị đích.
        if (document.hidden) {
     ...
     * @param {string} [message='Đang gửi']
     * @param {boolean} [replace=false]
     * @returns {object}
     */
    const disableButton = (button, message = 'Đang gửi', replace = false) => {
    ...
    const copy = async (button, message = null, timeout = 1500) => {
        ...
        try {
            ...
            notify('Sao chép không thành công').error();
            return;
        }
